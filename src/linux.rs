...
mod ffi {
    ...
    extern "C" {
        ...
        pub fn clock_nanosleep(clock: c_int, flags: c_int,
...
fn clock_gettime() -> SnoozeResult<timespec> {
    ...
    if ret != 0 {
        ...
        Err(match error.kind() {
            io::ErrorKind::InvalidInput => SnoozeError::Unsupported(
                "CLOCK_MONOTONIC is not supported".to_string()),
...
impl Snooze {
    pub fn new(duration: Duration) -> SnoozeResult<Snooze> {
        ...
        let duration_nanos =
            (duration - Duration::seconds(duration_secs)).num_nanoseconds().unwrap();
        Ok(Snooze {
            duration: timespec {
